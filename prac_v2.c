...
char	*validate_cmd_path(char *cmd_str)
{
	...
	if (!cmd_str || cmd_str[0] == '\0') // Check if input is empty string
	{
		write(2, "zsh: permission denied: \n", 25);
		return (NULL);
	}
	else if (ft_strcmp(cmd_str, " ") == 0) // Check if input is space only
	{
		write(2, "zsh: command not found: \n", 25);
		return (NULL);
	}

	if (!cmd_split || !cmd_split[0]) // Existing checks for null or invalid commands
	{
		write(2, "zsh: command not found:\n", 24);
		return (NULL);
	}
